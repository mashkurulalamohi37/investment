      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-slate-900 text-white p-6 relative">
          ...
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-bold">
              <Building2 className="w-3.5 h-3.5" /> BANK DEPOSIT & ESCROW
            </div>
            <h3 className="text-xl font-bold">Bank Transfer & Deposit Slip Upload</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4 text-xs">
            <div>
              <label className="font-bold text-slate-700 block mb-1">Depositor Full Name (জমাদানকারীর নাম)</label>
              <input
                type="text"
                required
                value={depositorName}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="font-bold text-slate-700 block mb-1">Deposit Bank</label>
                <input
                  type="text"
                  required
                  value={bankName}
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Slip / Txn Ref #</label>
                <input
                  type="text"
                  required
                  value={slipRef}
            ...
            <div>
              <label className="font-bold text-slate-700 block mb-1">Upload Deposit Receipt Photo (রসিদের ছবি)</label>
              <div className="p-4 border-2 border-dashed border-slate-200 rounded-2xl text-center space-y-2 hover:border-brand-forest transition-colors relative">
                {receiptImage ? (
                  <div className="space-y-2">
                    ...
                    <span className="text-[11px] text-jade font-semibold block">Photo attached successfully</span>
                  </div>
                ) : (
                  <>
                    <Upload className="w-8 h-8 text-slate-400 mx-auto" />
                    <span className="text-xs font-semibold text-slate-600 block">
                      Click to upload bank deposit slip photo
                    </span>
          ...
          <button
            type="submit"
            className="w-full py-4 rounded-xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 shadow-lg flex items-center justify-center gap-2 transition-all"
          >
            <span>Submit Slip for Admin Verification</span>
